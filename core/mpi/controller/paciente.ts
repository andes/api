...
// Buscar en elastic
export function matchingAuditoria(data) {
    let connElastic = new ElasticSync();
    let query;

    let campo = data.tipoBusqueda;
    let filter;
    if (campo === 'documento') {
        filter = data.documento;
    } else {
        filter = data.claveBlocking;
    }
    let condicionMatch = {};

    condicionMatch[campo] = filter,
        condicionMatch['estado'] = 'validado';
    // condicionMatch['activo'] = true;
    query = {
        condicionMatch
    };
    ...
    return new Promise((resolve, reject) => {
        ...
        connElastic.searchMultipleFields(condicionMatch)
            .then((searchResult) => {
                // Asigno los valores para el suggest
                let weights = config.mpi.weightsAuditoriaPacientes;

                if (data.escaneado) {
                    weights = config.mpi.weightsScan;
                }

                let porcentajeMatchMax = config.mpi.cotaMatchMaxAuditoria;
                let porcentajeMatchMin = config.mpi.cotaMatchMinAuditoria;
