...
export async function checkRepetido(nuevoPaciente, incluirTemporales = true): Promise<{ resultadoMatching: any[], dniRepetido: boolean, macheoAlto: boolean }> {
    ...
    for (let resultado of candidatos) {
        let idPaciente = Types.ObjectId(resultado.paciente.id);
...
function createMatchPatient(patient) {
...
export async function validarPaciente(pacienteAndes, req: any = configPrivate.userScheduler) {
    ...
    let resRenaper: any;
    try {
        resRenaper = await renaperv3({ documento: pacienteAndes.documento, sexo: sexoPaciente }, configPrivate.busInteroperabilidad, renaperToAndes);
        if (!resRenaper) {
        ...
        resRenaper.foto = (resRenaper.foto.includes('image/jpg')) ? await validarTamañoFoto(resRenaper.foto) : null;
...
export async function getHistorialPaciente(req, dataPaciente) {
    try {
        ...
        if (req.query.organizacion) {
            pipelineTurno.push({ $match: { 'organizacion._id': { $eq: new Types.ObjectId(Auth.getOrganization(req)) } } });
            pipelineSobreturno.push({ $match: { 'organizacion._id': { $eq: new Types.ObjectId(Auth.getOrganization(req)) } } });
        }
