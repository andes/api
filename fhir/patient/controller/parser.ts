...
export function pacienteAFHIR(id) {
    pacienteMpi.findById(id, function (err, data, next) {
        ...
        let relaciones = data.relaciones ? data.relaciones.map(unaRelacion => {
            let relacion = {
                "relationship": [{ text: unaRelacion.relacion.nombre }], // The kind of relationship
                "name": {
        ...
        let pacienteFHIR = {
            ...
            'maritalStatus': { text: data.EstadoCivil }, // Marital (civil) status of a patient
            "photo": [{ data: data.foto }], // Image of the patient
            'contact': relaciones
        }
