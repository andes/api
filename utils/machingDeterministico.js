...
var machingDeterministico = (function () {
     ...
     */
    machingDeterministico.prototype.stringMatching = function (stringA, stringB) {
        var stringAMin = stringA.toLowerCase();
        var stringBMin = stringB.toLowerCase();
        var maxLen = libString_1.libString.maxLargo(stringAMin, stringBMin);
        var minLen = libString_1.libString.minLargo(stringAMin, stringBMin);
        var coincidencias = 0;
     ...
     * @author Hugo Fernández hfernandez@neuquen.gov.ar
     */
    machingDeterministico.prototype.sexMatching = function (sexA, sexB) {
        if (sexA == sexB)
            return 1;
        else
            return 0;
     ...
     * @author Hugo Fernández hfernandez@neuquen.gov.ar
     */
    machingDeterministico.prototype.identityMatching = function (idA, idB) {
        if (idA == idB)
            return 1;
        else
            return 0;
     ...
     * @author Hugo Fernández hfernandez@neuquen.gov.ar
     */
    machingDeterministico.prototype.levenshtein = function (stringA, stringB) {
        //var s1 = libString.preprocessInput(stringA.toLowerCase());
        //var s2 = libString.preprocessInput(stringB.toLowerCase());
        var s1 = stringA;
        var s2 = stringB;
        var maxLen = libString_1.libString.maxLargo(s1, s2);
        var l1 = s1.length;
        var l2 = s2.length;
        var d = [];
        var c = 0;
     ...
     * @author Hugo Fernández hfernandez@neuquen.gov.ar
     */
    machingDeterministico.prototype.maching = function (identidadA, identidadB, weights) {
        var completeNameA = identidadA.firstname + identidadA.lastname;
        var completeNameB = identidadB.firstname + identidadB.lastname;
        var v1 = weights.name * this.levenshtein(libString_1.libString.preprocessInput(completeNameA.toLocaleLowerCase()), libString_1.libString.preprocessInput(completeNameB.toLowerCase()));
