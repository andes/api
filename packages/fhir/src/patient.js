var Patient = module.exports = {
    ...
    verify(patient) {
        let respuesta = true;
    let fieldVerified = Object.keys(patient).every(this.pacienteFHIRFields);
    ...
    if (fieldVerified) {
        ...
        if (patient.identifier.length > 0) {
            patient.identifier.forEach(anIdentifier => {
                respuesta = respuesta && Object.keys(anIdentifier).every(this.identifierFields);
        ...
        patient.name.forEach(aName => {
            respuesta = respuesta && this.validName(aName);
        });
        if (patient.active) {
        ...
        if (patient.telecom) {
            patient.telecom.forEach(aTelecom => {
                respuesta = respuesta && Object.keys(aTelecom).every(this.telecomFields);
        ...
        if (patient.address) {
            patient.address.forEach(anAddress => {
                respuesta = respuesta && Object.keys(anAddress).every(this.addressFields);
                ...
                if (anAddress.line) {
                    respuesta = respuesta && anAddress.line.every(this.areStrings);
                }
                if (anAddress.city) {
        ...
        if (patient.maritalStatus && patient.maritalStatus.text) {
            respuesta = respuesta && Object.keys(patient.maritalStatus).every(this.codingFields)
        ...
        if (patient.photo) {
            patient.photo.forEach(aPhoto => {
                respuesta = respuesta && Object.keys(aPhoto).every(this.photoFields);
                if (aPhoto.data) {
            ...
            patient.contact.forEach(aContact => {
                if (aContact.relationship) {
                    aContact.relationship.forEach(aRelation => {
                        respuesta = respuesta && Object.keys(aRelation).every(this.codingFields);
                    });
                }
                if (aContact.name) {
                    respuesta = respuesta && this.validName(aContact.name);
                }
            });
        }
    } else {
    ...
    validName(aName) {
        return Object.keys(aName).every(this.nameFields) && ('resourceType' in aName) && aName.resourceType === 'HumanName' &&
            ('family' in aName) && Array.isArray(aName.family) && aName.family.every(this.areStrings) &&
            ('given' in aName) && Array.isArray(aName.given) && aName.given.every(this.areStrings);
    },
