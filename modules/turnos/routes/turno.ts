                ...
                tipoPrestacion.findById(req.body.tipoPrestacion._id, function verificarTipoPrestacion(err, data) {
                    if (err) {
                        ...
                        agenda.findById(req.params.idAgenda, function getAgenda(err, data) {
                            ...
                            actualizarTurno(req, posBloque, posTurno, res, next, data, 'asignado');
                            if (req.body.turnoDoble) {
                                if ((data as any).bloques[posBloque].turnos[posTurno + 1].estado == 'disponible') {
                                    actualizarTurno(req, posBloque, posTurno + 1, res, next, data, 'turnoDoble');
                                }
                            }
                        });
                    }
...
function actualizarTurno(data, req, posBloque, posTurno, res, next, estado) {
    ...
    if (estado === 'asignado') {
        etiquetaTipoTurno = 'bloques.' + posBloque + '.turnos.' + posTurno + '.tipoTurno';
        ...
        datosOp['paciente'] = update[etiquetaPaciente];
        datosOp['prestacion'] = update[etiquetaPrestacion];
    }

    update[etiquetaEstado] = estado;
