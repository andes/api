...
export async function saveAgendaToHospital(agenda, pool) {
    ...
    async function saveTurnos(idAgendaAndes, turnos: Array<any>) {
        ...
        for (let turno of turnos) {
            if (turno.estado === constantes.EstadoTurnosAndes.asignado 
                ...
                let paciente = await pacientes.buscarPaciente(turno.paciente.id);

                if (await !existsPacienteHospital()) {
                    await pacienteHPN.savePaciente(paciente.paciente, pool);
                }
    ...
    function existsPacienteHospital() {
        return false;
    }
