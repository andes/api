...
EventCore.on('mpi:patient:update', async function (pacienteModified) {
    let req = {
        query: {
            estado: 'asignado',
        ...
        pipeline = [{
                $match: {
                    $and: [{
                        ...
                        {
                            'organizacion._id': {
                                '$eq': mongoose.Types.ObjectId(params.organizacion)
                            }
                        },
                        {
            ...
            {
                $unwind: {
                    path: '$diagnosticoCodificaciones',
