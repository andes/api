...
export async function buscarRecetas(req) {
    ...
    try {
        ...
        if (params.idRegistro) {
            options['idRegistro'] = params.idRegistro;
        }
...
async function aplicarRenovacion(recetaOriginal: any, profesionalData: any, organizacion: any, ahora: Date, req: any) {
    ...
    const recetaObj = recetaOriginal.toObject();
    // Se eliminan ids para que se generen nuevos
    delete recetaObj._id;
    delete recetaObj.id;
    const recetaNueva: any = new Receta(recetaObj);
    recetaNueva.idReceta = undefined;

    recetaNueva.profesional = profesionalData as any;
    recetaNueva.organizacion = {
    ...
    recetaNueva.fechaRegistro = moment(ahora).add(ordenTratamiento * 30, 'days').toDate();

    // Limpia origenExterno por si la receta original venía de otro lado
    recetaNueva.origenExterno = undefined as any;
    ...
    recetaNueva.estados = [{ tipo: nuevoEstado } as any];
    recetaNueva.estadosDispensa = [{ tipo: 'sin-dispensa', fecha: ahora } as any];
    ...
    recetaNueva.idRecetaOriginal = recetaOriginal.idRecetaOriginal || recetaOriginal._id.toString();
    recetaNueva.numeroRenovacion = numRenovacion;

    if (req.user) {
        Auth.audit(recetaNueva as any, req);
    } else {
        recetaNueva.audit(req);
    }
