...
function generarIdDesdeFecha(fecha = new Date()) {
    ...
    // Generar 3 caracteres aleatorios (números y letras)
    const caracteres = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    let caracteresAleatorios = '';
    for (let i = 0; i < 3; i++) {
        caracteresAleatorios += caracteres.charAt(Math.floor(Math.random() * caracteres.length));
    }

    const fechaString = fecha.getFullYear().toString() +
    ...
    return fechaString + caracteresAleatorios;
}
